...
// The grantable sections (must match the nav item data-v ids in the UI).
const SECTIONS = ['exec', 'fin', 'sched', 'tunnel', 'claims', 'inv', 'mat', 'ins', 'man', 'rsm', 'equip', 'safety', 'dpr'];
...
// Case-insensitive username lookup — returns the actual stored key that matches
// `name` regardless of case (or null). Usernames are matched, not case-sensitive.
...
const eqUser = (a, b) => !!a && !!b && String(a).toLowerCase() === String(b).toLowerCase();

async function currentUser(req) {
  const t = verifyToken(readToken(req));
  if (!t) return null;
  const users = await getUsers();
  const key = findUserKey(users, t.u);
  if (key) {
    const rec = users[key];
    return { username: key, isAdmin: !!rec.isAdmin, sections: rec.isAdmin ? SECTIONS.slice() : (rec.sections || []) };
  }
  if (eqUser(process.env.ADMIN_USER, t.u)) {
...
module.exports = {
  ...
  readToken, currentUser, sessionCookie, clearCookie, findUserKey, eqUser,
};
