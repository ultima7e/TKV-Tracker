...
function parseInsurance(matrices) {
  ...
  // transposed — a Description row of labels, then Percentage and Premium rows.
  const installments = (g) => {
    const di = g.findIndex((row) => row && /description/i.test(txt(row[0])));
    const mi = g.findIndex((row) => row && /^premium/i.test(txt(row[0])));
    if (di < 0 || mi < 0) return [];
    const hdr = g[di], pi = g.findIndex((row) => row && /percentage/i.test(txt(row[0]))), pct = pi >= 0 ? g[pi] : [], prem = g[mi];
    const out = [];
    for (let c = 1; c < hdr.length; c++) {
      const label = txt(hdr[c]); if (!label) continue;
      if ((pct[c] == null || pct[c] === '') && (prem[c] == null || prem[c] === '')) continue;
      out.push({ label, pct: (pct[c] == null || pct[c] === '') ? null : Math.round(num(pct[c]) * 100), premium: num(prem[c]) });
    }
    return out;
  };
  ...
  for (const r of summary) {
    ...
    if (Array.isArray(g)) {
      if (kv(g, /valid (from|till)/i) != null) {
        ...
        detail = { type: 'installments', currency: premiumUSD ? 'USD' : 'NPR',
          issueDate: iso(kv(g, /issue date/i)), items: installments(g) };
      } else {
