...
function parseInsurance(matrices) {
  ...
  // One register entry per group: total paid across its rows, and validity taken
  // from the row with the LATEST valid-till so renewed sets show current coverage.
  const policies = []; let totalPaid = 0;
  for (const grp of groups) {
    if (!grp.rows.length) continue;
    let best = null;
    for (const rr of grp.rows) if (rr.validTill && (!best || rr.validTill > best.validTill)) best = rr;
    const paidSum = grp.rows.reduce((s, rr) => s + (rr.paidNPR || 0), 0);
    totalPaid += paidSum;
    // Multi-row groups drill into their renewals; single policies (CAR, PI) drill
    // into their installment schedule from the detail sheet.
    let detail = null, currency = null, premiumUSD = 0;
    if (grp.rows.length > 1) {
      detail = { type: 'subpolicies', items: grp.rows };
    } else {
      const inst = installmentDetail(matrices[detailSheetName(grp.desc)]);
    ...
    policies.push({
      sn: grp.key, desc: grp.desc,
      policyNo: (grp.rows.find((rr) => rr.policyNo) || {}).policyNo || null,
      paidNPR: paidSum, validFrom: best ? best.validFrom : null, validTill: best ? best.validTill : null,
      remarks: best ? best.status : (grp.rows[0].status || null),
      currency, premiumUSD, group: grp.key, idx: policies.length, detail,
    });
  }
