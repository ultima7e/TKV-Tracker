...
function parseTunnelExcavation(matrices) {
  ...
  for (let i = 0; i < g.length; i++) {
    const r = g[i]; if (!r) continue;
    const area = txt(r[AREA]), loc = txt(r[LOC]), design = num(r[DESIGN]);
    ...
    if (loc && design != null) {                                             // Workfront header
      ...
      for (let j = i; j <= i + 3 && j < g.length; j++) {
        ...
        if (j > i && txt((g[j] || [])[LOC]) && num((g[j] || [])[DESIGN]) != null) break;
      }
      // Cumulative = the largest value in the Cum row (it only grows, so the max is
      // the latest — robust to completed fronts whose recent weekly cells are blank).
      let cum = 0;
      if (cumRow) for (const c of cumRow) { const v = num(c); if (v != null && v > cum) cum = v; }
      ...
      if (done) cum = Math.max(cum, design);
