...
function parseFinanceDetail(matrices) {
  ...
  // Advance amortisation: advance disbursed vs recovered (15% AP deductions on IPCs).
  let recoveredUSD = 0, recoveredNPR = 0;
  for (const r of grid) {
  ...
  return { budget, received, ipcs, earnedByCategory, advance: advanceSummary, warnings };
}
