...
// Insurance workbook. The 'Summary' sheet lists the main policies (S.N. ·
// Description · Paid Amount NPR + grand total); each policy's number, validity
// and premium live in its own detail sheet ('1.CAR Policy', '2. Professional
// Indemnity' as key-value blocks; '3. GPA', '4.P&E(CPM)' as tables).
function parseInsurance(matrices) {
  ...
  const txt = (v) => (v == null ? '' : String(v).replace(/\s+/g, ' ').trim());
  // key→value lookup down column A of a key-value detail sheet
  const kv = (grid, re) => { if (!Array.isArray(grid)) return null; const row = grid.find((r) => r && re.test(txt(r[0]))); return row ? row[1] : null; };
  const kvRow = (grid, re) => (Array.isArray(grid) ? grid.find((r) => r && re.test(txt(r[0]))) : null);
  const detailSheet = (desc) => {
    const d = desc.toLowerCase();
    if (/car\b|contractor'?s all risk/.test(d)) return '1.CAR Policy';
    if (/profession|indemnit/.test(d)) return '2. Professional Indemnity';
    if (/gpa/.test(d)) return '3. GPA';
    if (/plant|machinery|cpm|p&e|contractor/.test(d)) return '4.P&E(CPM)';
    return null;
  };

  // --- per-policy drill-down extractors (attached as policy.detail) ---
  // Installment payment schedule (CAR, Professional Indemnity): the block is
  // transposed — a Description row of labels, then Percentage, Paid-Amount (NPR),
  // Exchange-Rate and Status rows. The actual money paid is in NPR (the USD rows
  // are just the pre-conversion premium), so surface the NPR paid amount.
  const installments = (g) => {
    const findRow = (re) => g.find((row) => row && re.test(txt(row[0])));
    const hdr = findRow(/^description/i); if (!hdr) return [];
    const pct = findRow(/percentage/i) || [];
    const paid = findRow(/paid amount.*npr|paid.*\(in npr\)/i) || [];
    const exch = findRow(/exchange rate/i) || [];
    const stat = findRow(/^status/i) || [];
    const out = [];
    for (let c = 1; c < hdr.length; c++) {
      const label = txt(hdr[c]); if (!label) continue;
      const pd = num(paid[c]);
      if (!pd && (pct[c] == null || pct[c] === '')) continue;
      out.push({ label, pct: (pct[c] == null || pct[c] === '') ? null : Math.round(num(pct[c]) * 100),
        paidNPR: pd, exchangeRate: num(exch[c]) || null, status: txt(stat[c]) || null });
    }
    return out;
  };
  // GPA sub-policies grouped by insured group (Manpower / Employer / Army / Sub).
  const gpaSubs = (g) => {
    const hi = g.findIndex((row) => row && row.some((c) => /policy no/i.test(txt(c))));
    const h = g[hi] || [], col = (re) => h.findIndex((c) => re.test(txt(c)));
    const C = { pol: col(/policy no/i), ins: col(/insured/i), pay: col(/total payment/i), vf: col(/valid from/i), vt: col(/valid till/i), st: col(/remark|status/i) };
    const out = []; let group = '';
    for (let i = hi + 1; i < g.length; i++) {
      const row = g[i]; if (!row) continue; const a = txt(row[0]); if (!a) continue;
      if (!txt(row[C.pol])) { group = a; continue; } // group subheader
      out.push({ group, code: a, policyNo: txt(row[C.pol]), insuredNPR: num(row[C.ins]),
        paymentNPR: num(row[C.pay]), validFrom: iso(row[C.vf]), validTill: iso(row[C.vt]), status: txt(row[C.st]) });
    }
    return out;
  };
  // CPM / P&E insured equipment list.
  const equipment = (g) => {
    const h = g[0] || [], col = (re) => h.findIndex((c) => re.test(txt(c)));
    const C = { d: col(/description/i), m: col(/model|spec/i), q: col(/qty|quantity/i), ins: col(/insured/i), vf: col(/valid from/i), vt: col(/valid till/i) };
    const out = [];
    for (let i = 1; i < g.length; i++) {
      const row = g[i]; if (!row) continue; const d = txt(row[C.d < 0 ? 0 : C.d]); if (!d || /equipment under/i.test(d)) continue;
      out.push({ desc: d, model: txt(row[C.m]), qty: num(row[C.q]) || null, insuredNPR: num(row[C.ins]),
        validFrom: iso(row[C.vf]), validTill: iso(row[C.vt]) });
    }
    return out;
  };
  ...
  for (const r of summary) {
    if (!r) continue;
    const sn = txt(r[0]), desc = txt(r[1]), paid = num(r[2]);
    if (/^total$/i.test(desc)) { totalPaid = paid || totalPaid; continue; } // grand-total row
    if (!/^\d+$/.test(sn) || !desc) continue; // only numbered policy rows
    const g = matrices[detailSheet(desc)];
    let policyNo = null, validFrom = null, validTill = null, premiumUSD = 0, premiumNPR = 0, detail = null;
    if (Array.isArray(g)) {
      if (kv(g, /valid (from|till)/i) != null) {
        // key-value detail sheet (CAR, Professional Indemnity): labels in col A
        policyNo = txt(kv(g, /policy no/i)) || null;
        validFrom = iso(kv(g, /valid from/i));
        validTill = iso(kv(g, /valid till/i));
        const pr = kvRow(g, /total premium/i);
        if (pr) { if (/usd/i.test(txt(pr[0]))) premiumUSD = num(pr[1]); else premiumNPR = num(pr[1]); }
        detail = { type: 'installments', issueDate: iso(kv(g, /issue date/i)), items: installments(g) };
      } else {
        // table detail sheet (GPA, P&E/CPM): find the header row, then summarise.
        // Policy no from the first data row; validity from the row with the LATEST
        // valid-till, so renewed sets (e.g. GPA) show current coverage, not an old
        // expired row.
        const hi = g.findIndex((row) => row && row.some((c) => /policy no|code no/i.test(txt(c))));
        const header = (hi >= 0 ? g[hi] : g[0]) || [];
        const col = (re) => header.findIndex((c) => re.test(txt(c)));
        const pc = col(/policy no/i) >= 0 ? col(/policy no/i) : (col(/code no/i) >= 0 ? col(/code no/i) : 2);
        const vf = col(/valid from/i), vt = col(/valid till/i);
        const dataRows = g.slice((hi >= 0 ? hi : 0) + 1).filter((row) => row && txt(row[pc]));
        if (dataRows.length) {
          policyNo = txt(dataRows[0][pc]) || null;
          if (vt >= 0) {
            let best = null;
            for (const row of dataRows) { const t = iso(row[vt]); if (t && (!best || t > best.till)) best = { till: t, from: vf >= 0 ? iso(row[vf]) : null }; }
            if (best) { validTill = best.till; validFrom = best.from; }
          }
        }
        detail = /gpa/i.test(desc) ? { type: 'subpolicies', items: gpaSubs(g) } : { type: 'equipment', items: equipment(g) };
      }
    }
    policies.push({ sn, desc, policyNo, paidNPR: paid, validFrom, validTill,
      premiumUSD, premiumNPR, remarks: null, group: sn, idx: policies.length, detail });
  }
  if (!totalPaid) totalPaid = policies.reduce((s, p) => s + p.paidNPR, 0);
