...
function parseInsurance(matrices) {
  ...
  for (const grp of groups) {
    ...
    policies.push({
      ...
      group: grp.key, idx: policies.length,
      // Multi-row groups (renewals / sub-policies) drill down; single policies don't.
      detail: grp.rows.length > 1 ? { type: 'subpolicies', items: grp.rows } : null,
    });
  }
