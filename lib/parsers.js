...
function parseInsurance(matrices) {
  ...
  for (let i = hIdx + 1; i < grid.length; i++) {
    ...
    if (!validTill && !paid) continue; // skip pure category headers
    policies.push({ sn, desc, policyNo: txt(r[2]) || null, paidNPR: paid,
      validFrom: iso(r[7]), validTill, remarks: txt(r[11]) || null });
  }
  if (!totalPaid) totalPaid = policies.reduce((s, p) => s + p.paidNPR, 0);
  return { warnings, policies, totalPaidNPR: Math.round(totalPaid * 100) / 100 };
}
