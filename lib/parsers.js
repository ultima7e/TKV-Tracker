  ...
  for (const r of grid) {
    ...
    if (/^IPC-\d+/i.test(d)) {
      ...
      advance.items.push({ code: d, category: 'Advance Payment',
        paymentPct: typeof r[1] === 'number' ? Math.round(r[1] * 100) : null, netUSD: num(r[21]), netNPR: num(r[22]) });
    ...
    } else if (/ipcs|^total\b|^actual received|^certified\b|^net amount|^grand/i.test(d)) {
      // grand-total / summary rows at the foot of the sheet — not IPC items
      continue;
    } else {
      const t = cur || advance;
      if (t && (num(r[21]) || num(r[22]) || typeof r[1] === 'number')) {
        t.items.push({ code: d, category: CAT[d[0].toUpperCase()] || 'Other',
          paymentPct: typeof r[1] === 'number' ? Math.round(r[1] * 100) : null,
          netUSD: num(r[21]), netNPR: num(r[22]) });
      }
    }
  }
