...
function readEV(matrices) {
  ...
  // EV Front summary block (below the main table). The file expresses the
  // headline "Total Earned Value" two ways using its own exchange rate: an
  // all-in USD-equivalent (col C = USD portion + NPR portion ÷ rate) and an
  // all-in NPR-equivalent (col D = USD portion × rate + NPR portion). Both live
  // on the row directly beneath the "Total Earned Value" header, so they update
  // whenever the workbook is edited. The rate here is the file's own (currently
  // 151) and is intentionally separate from EV_RATE used elsewhere.
  let totalEarnedUSDeq = null, totalEarnedNPReq = null, evRate = null;
  for (let i = 0; i < evFront.length; i++) {
    ...
    if (ti >= 0) {
      ...
      if (typeof vr[ti] === 'number') totalEarnedUSDeq = vr[ti];
      if (typeof vr[ti + 1] === 'number') totalEarnedNPReq = vr[ti + 1];
    }
  }
  ...
  const budget = {
    ...
    // Headline "Total Earned Value" from the EV Front summary block, both ways
    // (USD-equivalent and NPR-equivalent), at the file's own exchange rate.
    totalEarnedUSDeq, totalEarnedNPReq, evRate,
  };
