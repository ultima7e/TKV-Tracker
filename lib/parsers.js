...
function parseTunnelExcavation(matrices) {
  ...
  const sheetName = names.find((n) => /^[A-Za-z]{3,9}[-\s]?20\d\d$/.test(String(n).trim())) || names[0];
  const g = matrices[sheetName];
  if (!g || !g.length) { warnings.push('Tunnel excavation: no month data sheet found'); return { warnings, missing: true }; }
  const num = (v) => (typeof v === 'number' ? v : (v == null || v === '' ? null : (parseFloat(String(v).replace(/[^0-9.\-]/g, '')) || null)));
  const txt = (v) => (v == null ? '' : String(v).replace(/\s+/g, ' ').trim());
  const rnd = (v, d) => (v == null ? null : Math.round(v * 10 ** d) / 10 ** d);
  const iso = (s) => { const n = typeof s === 'number' ? s : parseFloat(s); return (n > 30000 && n < 80000) ? new Date(Math.round((n - 25569) * 86400 * 1000)).toISOString().slice(0, 10) : null; };
  const areas = []; let cur = null;
  for (let i = 0; i < g.length; i++) {
    const r = g[i]; if (!r) continue;
    const b = txt(r[1]), a = txt(r[0]), desc = txt(r[6]);
    if (a && !b && !desc && num(r[5]) == null) { cur = { name: a, workfronts: [] }; areas.push(cur); continue; } // Area header
    if (b && num(r[5]) != null) {                                            // Workfront header
      const design = num(r[5]), uom = txt(r[4]) || 'm';
      let cum = 0;
      for (let j = i + 1; j <= i + 3 && j < g.length; j++) {
        if (/cum/i.test(txt((g[j] || [])[6]))) { cum = Math.max(num((g[j] || [])[14]) || 0, num((g[j] || [])[15]) || 0); break; }
        if (txt((g[j] || [])[1])) break;
      }
      const pct = design ? Math.min(100, Math.round((cum / design) * 1000) / 10) : 0;
      if (!cur) { cur = { name: 'Tunnels', workfronts: [] }; areas.push(cur); }
      cur.workfronts.push({
        loc: b, uom, design: rnd(design, 2), excavated: rnd(cum, 2), pct,
        status: pct <= 0 ? 'Not Started' : pct >= 99.95 ? 'Complete' : 'In Progress',
        chStart: rnd(num(r[2]), 2), chEnd: rnd(num(r[3]), 2),
        planStart: iso(r[8]), planFinish: iso(r[9]), actualStart: iso(r[11]), actualFinish: iso(r[12]),
      });
    }
  }
  ...
  return {
    warnings, sheet: sheetName, areas: res,
    counts: {
      total: all.length,
