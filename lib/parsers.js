...
function parseInsurance(matrices) {
  const warnings = [];
  const grid = matrices['Summary'];
  if (!grid) { warnings.push("Insurance: 'Summary' sheet not found"); return { warnings, missing: true }; }
  const num = (v) => (typeof v === 'number' ? v : 0);
  const iso = (s) => (typeof s === 'number' && s > 30000 && s < 80000
    ? new Date(Math.round((s - 25569) * 86400 * 1000)).toISOString().slice(0, 10)
    : (typeof s === 'string' && /\d{4}-\d\d-\d\d/.test(s) ? s.slice(0, 10) : null));
  const txt = (v) => (v == null ? '' : String(v).replace(/\s+/g, ' ').trim());
  let hIdx = grid.findIndex((r) => r && r.some((c) => /valid till/i.test(String(c || ''))));
  if (hIdx < 0) hIdx = 1;
  const policies = []; let totalPaid = 0;
  const categories = {}; // top-level S.N. -> category name (e.g. "6" -> "Plant and Machinery")
  for (let i = hIdx + 1; i < grid.length; i++) {
    const r = grid[i]; if (!r) continue;
    const sn = txt(r[0]), desc = txt(r[1]);
    const paid = num(r[3]), validTill = iso(r[8]);
    if (!desc) { if (!sn && paid > totalPaid) totalPaid = paid; continue; } // grand-total row
    if (!validTill && !paid) { if (/^\d+$/.test(sn)) categories[sn] = desc; continue; } // category header row
    const gi = parseInt(sn, 10);
    const group = Number.isFinite(gi) ? String(gi) : (sn || 'other'); // group = top-level S.N.
    policies.push({ sn, desc, policyNo: txt(r[2]) || null, paidNPR: paid,
      validFrom: iso(r[7]), validTill, remarks: txt(r[11]) || null, group, idx: policies.length });
  }
