...
function parseClaimsRegister(matrices) {
  ...
  const readItems = (name, kind) => {
    ...
    for (let i = 2; i < g.length; i++) {
      ...
      const no = txt(r[0]);
      if (no) {
        cur = { ref: no, kind, title: txt(r[1]), description: txt(r[2]), basis: txt(r[3]),
  ...
  if (!claims.length && !variations.length) { warnings.push('Claims register: no rows in Claim/Variation sheets'); return { warnings, missing: true }; }
  const all = [...claims, ...variations];
  all.forEach((it) => it.letters.sort((a, b) => String(a.date || '').localeCompare(String(b.date || ''))));
  ...
  return { warnings, claims, variations, kpis };
}
