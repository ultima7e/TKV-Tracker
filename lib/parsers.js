...
function parseIpcItems(matrices) {
  ...
  const serialToIso = (s) =>
    (typeof s === 'number' && s > 30000 && s < 80000
      ? new Date(Math.round((s - 25569) * 86400 * 1000)).toISOString().slice(0, 10) : null);
  // Payment % cells hold a fraction (0.33 = 33%); guard footer rows that put an amount there.
...
function parseClaims(matrices) {
  ...
  const claims = [], eot = [];
  let cur = null, socCount = 0, approvedNPR = 0, maxEot = 0;
  for (let i = 1; i < summary.length; i++) {
    ...
    if (no) {
      ...
      const rec = {
        no: labelOf(no), subject, basis: basisOf(subject, txt(r[4])),
      ...
      if (/eot/i.test(no)) { if (rec.eotDays) maxEot = Math.max(maxEot, rec.eotDays); eot.push(rec); cur = rec; }
  ...
  return {
    ...
    socCount, approvedNPR: approvedNPR / 1e6, eotDays: maxEot || null,
