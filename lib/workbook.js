...
function workbookSheets(buffer, names) {
  ...
  for (const name of names) {
    // Resolve to the actual sheet even if its tab name has stray leading/
    // trailing spaces or differing case (e.g. "Potential Claim "); store the
    // matrix under the clean requested name so parsers can use tidy keys.
    const key = wb.Sheets[name] ? name : wb.SheetNames.find((n) => norm(n) === norm(name));
    if (key && wb.Sheets[key]) matrices[name] = XLSX.utils.sheet_to_json(wb.Sheets[key], { header: 1, defval: null });
  }
  return { matrices };
}
