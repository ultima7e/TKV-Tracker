...
// Material approvals — the "Summary of Material Approval List" workbook (material
...
// B.5 Regular Service & Maintenance cost workbook — hosted on Dropbox, fetched
// live and parsed in ISOLATION (only its 'Pivot Table' sheet) so its 20 mixed
// sheets never merge into the finance/generic bag.
const RSM_DBX_URL = 'https://www.dropbox.com/scl/fi/ieqasqonv7h2nnq2616a0/Subactivity-Percentage-Completion-Calculation_B.5-Regular-Service-Maintenance.xlsx?rlkey=s7mfrx59y9htxvxlq610yvixh&dl=1';
...
function rsmFromBuffer(buffer) {
  if (!buffer) return { missing: true, warnings: ['RSM cost workbook not available'] };
  try {
    const { matrices } = workbookSheets(buffer, ['Pivot Table']);
    return parseRsm(matrices);
  } catch (e) {
    return { missing: true, warnings: ['RSM parse failed: ' + String(e.message || e)] };
  }
}
...
// Material approvals workbook — parsed in isolation (six registers in one file), so
// its many sheets never merge into the finance bag.
function materialsFromBuffer(buffer) {
  if (!buffer) return { missing: true, warnings: ['Material approval workbook not available'] };
  try {
    const { matrices } = workbookToBoth(buffer);
    return parseMaterials(matrices);
  } catch (e) {
    return { missing: true, warnings: ['Materials parse failed: ' + String(e.message || e)] };
  }
}
...
async function buildPayload() {
  ...
  if (headers && process.env.NUTSTORE_FILE_PATH) {
    ...
    // Newest material-approval workbook in the Material Approval folder.
    const matFile = (matInfo || [])
      .filter((e) => e && e.path && MAT_RE.test(e.path) && !/\/~\$/.test(e.path))
      .sort((a, b) => new Date(b.mtime || 0) - new Date(a.mtime || 0))[0];
    const matPath = matFile ? matFile.path : '';
    const matMtime = matFile ? matFile.mtime : '';
  ...
  const rsmDbx = await dbxFetch(RSM_DBX_URL).catch(() => null);
