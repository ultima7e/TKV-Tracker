(() => {
  ...
  // S-Curve 2 — TKV Monthly Planned Progress (cost-based), revised EOT-final
  // baseline (TKV_Planned_SCurve_EOT_Final.xlsx, Jun-2024 → Nov-2029). Embedded
  // as a fixed planned baseline; it is not part of the live Nutstore feed.
  const SCURVE2 = {
    ...
    monthly: [2.41,1.65,0.86,0.65,1.5,1.71,1.86,0.34,0.2,0.05,0.45,0.55,0.94,0.91,0.92,0.89,0.42,0.54,0.43,0.68,0.28,0.43,1.87,2.53,3.57,3.25,1.91,2.42,2.9,2.82,2.63,3.35,2.82,2.66,2.25,2.74,3.59,3.51,2.82,3.13,4.16,2.77,3.01,2.69,2.36,2.51,2.64,3.05,1.63,1.34,1.2,0.68,0.76,0.48,0.45,0.08,0.07,0.05,0.09,0.1,0.07,0.11,0.11,0.09,0.04,0.03],
  ...
  function renderSCurve2(id) {
    const RED = '#e5413f';
    makeChart(id).setOption({
      grid: { left: 46, right: 46, top: 22, bottom: 58 },
      legend: { top: 0, right: 0, textStyle: { fontSize: 11, color: COL.muted } },
      tooltip: { trigger: 'axis', formatter: (ps) => {
        let mo = null, cu = null;
        ps.forEach((p) => { if (p.seriesName === 'Planned Monthly %') mo = p.value; if (p.seriesName === 'Planned Cumulative %') cu = p.value; });
        const dot = (c) => `<span style="display:inline-block;width:8px;height:8px;border-radius:50%;background:${c};margin-right:6px"></span>`;
        return `<b>${ps[0].axisValue}</b><br/>${dot(COL.accent)}Monthly: ${mo == null ? '—' : mo + '%'}<br/>${dot(RED)}Cumulative: ${cu == null ? '—' : cu + '%'}`;
      } },
      xAxis: { type: 'category', data: SCURVE2.months,
        axisLabel: { fontSize: 8, color: COL.muted, interval: 0, rotate: 90 },
        axisLine: { lineStyle: { color: '#cfd8e6' } } },
      yAxis: [
        { type: 'value', name: 'Monthly %', nameTextStyle: { fontSize: 10, color: COL.muted },
          axisLabel: { fontSize: 10, color: COL.muted, formatter: '{value}%' },
          splitLine: { lineStyle: { color: COL.grid } } },
        { type: 'value', name: 'Cumulative %', min: 0, max: 100,
          nameTextStyle: { fontSize: 10, color: COL.muted },
          axisLabel: { fontSize: 10, color: COL.muted, formatter: '{value}%' },
          splitLine: { show: false } },
      ],
      series: [
        { name: 'Planned Monthly %', type: 'bar', yAxisIndex: 0, data: SCURVE2.monthly,
          itemStyle: { color: COL.accent }, barWidth: '62%' },
        { name: 'Planned Cumulative %', type: 'line', yAxisIndex: 1, smooth: true,
          symbol: 'circle', symbolSize: 3, data: SCURVE2.cumulative,
