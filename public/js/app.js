(() => {
  ...
  function renderSCurve(id) {
    ...
    makeChart(id).setOption({
      ...
      tooltip: { trigger: 'axis', formatter: (ps) => {
        let pl = null, ac = null, eo = null;
        ps.forEach((p) => { if (p.seriesName === 'Planned') pl = p.value; if (p.seriesName === 'Actual') ac = p.value; if (p.seriesName === 'Planned (EOT-final)') eo = p.value; });
        const dot = (c) => `<span style="display:inline-block;width:8px;height:8px;border-radius:50%;background:${c};margin-right:6px"></span>`;
        let s = `<b>${ps[0].axisValue}</b><br/>${dot(COL.accent)}Planned: ${pl == null ? '—' : pl + '%'}<br/>${dot(COL.accent2)}Actual: ${ac == null ? '—' : ac + '%'}<br/>${dot(EOT)}Planned (EOT): ${eo == null ? '—' : eo + '%'}`;
        if (pl != null && ac != null) {
          const v = Math.round((ac - pl) * 10) / 10;
          const behind = v < 0;
      ...
      series: [
        ...
        { name: 'Planned (EOT-final)', type: 'line', smooth: true, symbol: 'none',
          data: eot, connectNulls: true,
          lineStyle: { width: 2.5, color: EOT, type: 'dashed' } },
      ],
    });
  }
  ...
  // Progress S-Curve as the "Planned (EOT-final)" line.
  const SCURVE2 = {
